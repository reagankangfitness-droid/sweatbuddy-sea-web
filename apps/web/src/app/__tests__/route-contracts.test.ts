...
describe('route contracts', () => {
  it('keeps the public homepage pointed at the discovery command center', () => {
    ...
    expect(buddyPage).toContain('communityPins')
    expect(buddyPage).not.toContain('supportPlaces')
    ...
    expect(placePage).toContain("redirect('/communities?city=singapore')")
    expect(placePage).toContain('index: false')
    expect(placePage).not.toContain('Open official link')
    expect(placePage).not.toContain('Suggest official page')
    expect(placePage).not.toContain('sourceConfidenceScore')
    expect(singaporePage).toContain('Fitness Communities in Singapore')
    expect(singaporePage).toContain('CityLandingPage')
    expect(singaporePage).toContain('singaporeLanding')
    expect(singaporePage).not.toContain('FitnessDirectoryPage')
  ...
  it('keeps the public map community-only while preserving place infrastructure off the main surface', () => {
    const buddyPage = readRepoFile('apps/web/src/app/buddy/page.tsx')
    ...
    expect(buddyPage).not.toContain('/api/map/places?')
    expect(buddyPage).toContain('communityPins')
    expect(buddyPage).not.toContain('placePins')
    expect(buddyPage).toContain('sessionPins')
    expect(buddyPage).not.toContain('MapSelectedPlaceCard')
    expect(buddyPage).toContain('community activity')
    expect(buddyPage).not.toContain('Meetup spots')
    ...
    expect(buddyPage).not.toContain('sessions.length > 0 ? 18 : 36')
    ...
    expect(placePage).toContain("redirect('/communities?city=singapore')")
    expect(placePage).toContain('index: false')
    expect(placePage).not.toContain('mt-14 max-w-4xl')
  })
