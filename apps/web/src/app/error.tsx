      ...
      <div className="text-center max-w-md">
        ...
        <div className="space-y-3">
          ...
          <button
            ...
            className="w-full px-6 py-3 bg-white text-[#1A1A1A] border border-black/[0.08] rounded-full font-medium hover:bg-[#FFFBF8] transition-colors"
          >
            Go back home
          </button>
        </div>
        ...
        {process.env.NODE_ENV === 'development' && (
          <details className="mt-6 text-left">
            <summary className="text-xs text-[#9A9AAA] cursor-pointer">
              Error details
            </summary>
            <pre className="mt-2 p-3 bg-white border border-black/[0.06] rounded-lg text-xs text-red-500 overflow-auto max-h-40">
              {error.message}
              {error.digest && `\n\nDigest: ${error.digest}`}
            </pre>
          </details>
