  ...
  return (
    <div className="min-h-screen bg-neutral-900">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
        ...
        {/* Section Nav Pills */}
        {hasData && (
          <div className="sticky top-[57px] z-30 -mx-4 sm:-mx-6 px-4 sm:px-6 py-2 bg-neutral-900/95 backdrop-blur-lg border-b border-neutral-800 mb-6">
        ...
        {!hasData ? (
          <div className="bg-neutral-950 rounded-2xl border border-neutral-800 p-12 text-center">
          ...
          <>
            ...
            <div ref={overviewRef} id="overview" className="scroll-mt-28 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4 mb-8">
              ...
              <StatCard
                ...
                label="Show-up Rate"
                color="bg-purple-900 text-purple-600"
              />
              <StatCard
            ...
            {data.insights && (data.insights.bestDay || data.insights.bestTime) && (
              <div className="bg-gradient-to-r from-blue-50 to-purple-50 border border-blue-100 rounded-xl p-4 sm:p-6 mb-8">
                <div className="flex items-start gap-3">
                  <div className="w-10 h-10 bg-neutral-950 rounded-full flex items-center justify-center flex-shrink-0">
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                        ...
                        {rec.type === 'success' && <CheckCircle className="w-4 h-4 text-green-400" />}
                        {rec.type === 'warning' && <AlertTriangle className="w-4 h-4 text-amber-400" />}
                        {rec.type === 'info' && <Info className="w-4 h-4 text-blue-400" />}
                      </div>
                      <div>
                        <h4 className="font-semibold text-neutral-100 text-sm">{rec.title}</h4>
            ...
            {(data.upcomingEventPredictions?.length > 0 || data.revenueForecast?.length > 0) && (
              <div className="mb-8">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h2 className="font-semibold text-lg text-neutral-100">Predictions</h2>
                  <span className="text-xs bg-purple-900 text-purple-700 px-2 py-0.5 rounded-full">AI Powered</span>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="bg-neutral-950 rounded-xl border border-neutral-800 overflow-hidden">
                      ...
                      <div className="divide-y divide-neutral-800">
                        {data.upcomingEventPredictions.map((event) => (
                          <div key={event.id} className="px-4 py-3 hover:bg-neutral-900">
                            <div className="flex items-center justify-between">
                              <div className="min-w-0 flex-1">
                                <p className="font-medium text-neutral-100 truncate">{event.name}</p>
                              ...
                              <div className="text-right ml-4">
                                ...
                                <p className={`text-xs ${
                                  event.predictedShowUpRate >= 80 ? 'text-green-400' :
                                  event.predictedShowUpRate >= 60 ? 'text-amber-400' :
                                  'text-red-400'
                                }`}>
                  ...
                  {data.revenueForecast && data.revenueForecast.length > 0 && data.summary.totalRevenue > 0 && (
                    <div className="bg-neutral-950 rounded-xl border border-neutral-800 overflow-hidden">
                      <div className="px-4 py-3 border-b border-neutral-800">
                        <h3 className="font-semibold text-neutral-100">Revenue Forecast</h3>
                        ...
                        <div className="flex items-end justify-between gap-4">
                          {data.revenueForecast.map((forecast, idx) => (
                            <div key={forecast.month} className="flex-1 text-center">
                              <div className="relative h-24 bg-neutral-800 rounded-lg overflow-hidden mb-2">
                                <div
                              ...
                              <p className="text-lg font-bold text-neutral-100">${forecast.predictedRevenue.toFixed(0)}</p>
                              <p className="text-xs text-neutral-500">{forecast.month}</p>
                              <span className={`text-xs px-1.5 py-0.5 rounded ${
                                forecast.confidence === 'high' ? 'bg-green-900 text-green-400' :
                                forecast.confidence === 'medium' ? 'bg-amber-900 text-amber-400' :
                  ...
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-neutral-800">
                      {data.topEvents.map((event, index) => (
                        <tr key={event.id} className="hover:bg-neutral-900">
                          <td className="px-4 sm:px-6 py-4">
                            <div className="flex items-center gap-3">
                              <span className="w-6 h-6 bg-neutral-800 rounded-full flex items-center justify-center text-xs font-bold text-neutral-500">
                                {index + 1}
                              </span>
                              <div className="min-w-0">
                                <p className="font-medium text-neutral-100 truncate max-w-[200px] sm:max-w-none">
                                  {event.name}
                                </p>
                                {event.date && (
                                  <p className="text-xs text-neutral-500">
                          ...
                          <td className="px-4 sm:px-6 py-4 text-right font-semibold text-neutral-100">
                            {event.rsvps}
                          </td>
                          <td className="px-4 sm:px-6 py-4 text-right text-neutral-400 hidden sm:table-cell">
                            {event.attended}
                          </td>
                          <td className="px-4 sm:px-6 py-4 text-right">
                            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${
                              event.showUpRate >= 80 ? 'bg-green-900 text-green-400' :
                              event.showUpRate >= 60 ? 'bg-amber-900 text-amber-400' :
                              'bg-red-900 text-red-400'
                            }`}>
                              {event.showUpRate}%
                            </span>
...
function ChartCard({
  ...
  return (
    <div className="bg-neutral-950 rounded-xl border border-neutral-800 p-4 sm:p-6">
      <div className="mb-4">
