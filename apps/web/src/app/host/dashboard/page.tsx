      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-12">
        ...
        {data.stats.pendingPayments && data.stats.pendingPayments > 0 && (
          <Link href="/host/payments" className="block mb-6 sm:mb-8">
            <div className="p-3 sm:p-4 bg-amber-950 border border-amber-300 rounded-xl hover:bg-amber-900 transition-colors">
                  ...
                  >
                    ...
                    {tab.count > 0 && (
                      <span className={`px-1.5 py-0.5 text-xs rounded-full ${
                        activeTab === tab.id ? 'bg-white/20 text-white' : 'bg-neutral-700 text-neutral-400'
                      }`}>
                        {tab.count}
                      </span>
                    )}
                ...
                <div className="border border-neutral-800 rounded-xl overflow-hidden bg-neutral-950">
                  <div className="divide-y divide-neutral-800">
                    {data.recentActivity.slice(0, 6).map((activity) => (
                      <div key={activity.id} className="p-2.5 sm:p-3 hover:bg-neutral-900 transition-colors">
                        <div className="flex items-start gap-2.5 sm:gap-3">
                          <div className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                            activity.type === 'paid' ? 'bg-green-900' : 'bg-blue-900'
                          }`}>
                            {activity.type === 'paid' ? (
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-xs sm:text-sm text-neutral-100 truncate">
                  ...
                  {data.recentActivity.length > 6 && (
                    <Link
                      href="/host/community"
                      className="block text-xs text-neutral-400 hover:text-neutral-100 py-2.5 text-center font-medium border-t border-neutral-800 bg-neutral-900 hover:bg-neutral-800 transition-colors"
                    >
                      View all activity →
                    </Link>
                  )}
                    ...
                    {data.topRegulars.slice(0, 5).map((regular, index) => (
                      <div key={regular.email} className="p-2.5 sm:p-3">
                        <div className="flex items-center gap-2.5 sm:gap-3">
                          <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-amber-900 flex items-center justify-center flex-shrink-0">
  ...
  return (
    <div className="p-3 sm:p-4">
      <div className="flex items-start gap-3 sm:gap-4">
        <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-amber-900 flex items-center justify-center flex-shrink-0">
          <Clock className="w-4 h-4 sm:w-5 sm:h-5 text-amber-400" />
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-start gap-2 flex-wrap">
            <h3 className="font-semibold text-neutral-100 text-sm sm:text-base truncate">{event.name}</h3>
            <span className="inline-flex items-center px-2 py-0.5 text-xs font-medium rounded-full bg-amber-900 text-amber-400 flex-shrink-0">
              Pending
            </span>
          </div>
...
function RejectedEventRow({ event }: { event: DashboardEvent }) {
  return (
    <div className="p-3 sm:p-4">
      <div className="flex items-start gap-3 sm:gap-4">
        <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-red-900 flex items-center justify-center flex-shrink-0">
          <XCircle className="w-4 h-4 sm:w-5 sm:h-5 text-red-400" />
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-start gap-2 flex-wrap">
            <h3 className="font-semibold text-neutral-100 text-sm sm:text-base truncate">{event.name}</h3>
            <span className="inline-flex items-center px-2 py-0.5 text-xs font-medium rounded-full bg-red-900 text-red-400 flex-shrink-0">
              Rejected
            </span>
          </div>
          <p className="text-xs sm:text-sm text-neutral-500 mt-0.5 sm:mt-1">
...
function CancelledEventRow({ event }: { event: DashboardEvent }) {
  return (
    <div className="p-3 sm:p-4">
      <div className="flex items-start gap-3 sm:gap-4">
        <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-neutral-700 flex items-center justify-center flex-shrink-0">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start gap-2 flex-wrap">
            ...
            <span className="inline-flex items-center px-2 py-0.5 text-xs font-medium rounded-full bg-neutral-700 text-neutral-400 flex-shrink-0">
              Cancelled
            </span>
          </div>
