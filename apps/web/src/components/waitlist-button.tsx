...
export function WaitlistButton({
  ...
  if (waitlistStatus === 'NOTIFIED') {
    return (
      <div
        className={cn(
          'bg-gradient-to-r from-amber-50 to-amber-100 border-2 border-amber-400 rounded-xl p-4',
          className
        )}
      >
        <div className="flex items-center gap-3">
          <div className="w-11 h-11 rounded-xl bg-amber-500 text-white flex items-center justify-center animate-bounce">
            <BellRing size={20} />
          </div>
          <div>
            <div className="text-base font-bold text-amber-800">A spot opened up!</div>
            <div className="text-sm text-amber-700">Book now before it&apos;s gone</div>
          </div>
        </div>
        <div className="text-xs text-amber-700 mt-3 pt-3 border-t border-amber-300/50">
  ...
  if (isOnWaitlist) {
    return (
      <div
        className={cn(
          'flex items-center justify-between gap-3 px-4 py-3 bg-green-50 border border-green-200 rounded-xl',
          className
        )}
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-sm font-semibold text-green-800">You&apos;re on the waitlist!</div>
        ...
        <button
          ...
          disabled={loading}
          className="flex items-center gap-1.5 text-sm font-medium px-3 py-2 rounded-lg border border-green-300 text-green-700 hover:bg-green-100 transition-colors disabled:opacity-50"
        >
...
export function WaitlistStatus({
  ...
  return (
    <div
      className={cn(
        'inline-flex items-center gap-2 text-sm text-green-700 bg-green-50 px-3 py-1.5 rounded-lg',
        className
      )}
    >
      <Check size={14} />
