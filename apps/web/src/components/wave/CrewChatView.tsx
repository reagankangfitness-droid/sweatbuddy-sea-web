    ...
    <AnimatePresence>
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-neutral-800">
          ...
          <span className="font-semibold text-neutral-100">{area}</span>
        </div>
          ...
          <div className="flex items-center justify-center py-2">
            <div className="flex items-center">
              {participants.slice(0, 4).map((p, i) => (
                <div
                  key={p.userId}
                  className="w-10 h-10 rounded-full overflow-hidden bg-neutral-700 ring-2 ring-white flex items-center justify-center"
        ...
        {/* Pinned thought card */}
        {starterThought && (
          <div className="mx-4 mt-2 mb-1 flex items-start gap-2 p-3 bg-neutral-900 border border-neutral-800 rounded-xl">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold text-neutral-300">{starterName || 'Anonymous'}</p>
              <p className="text-sm text-neutral-400 mt-0.5">{starterThought}</p>
              ...
              <div className="flex flex-wrap gap-2 justify-center">
                {STARTERS.map((s) => (
                  <button
                    ...
                    className="px-3 py-2 rounded-full bg-neutral-800 text-sm text-neutral-300 border border-neutral-800 hover:bg-neutral-700 transition-colors"
                  >
                    {s}
                  </button>
                ))}
            ...
            return (
              <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[75%] ${!isMe ? 'flex gap-2' : ''}`}>
                  {!isMe && (
                    <div className="w-7 h-7 rounded-full overflow-hidden bg-neutral-700 shrink-0 mt-1">
                      {msg.senderImageUrl ? (
        ...
        {/* Input */}
        <div className="px-4 py-3 border-t border-neutral-800 pb-[env(safe-area-inset-bottom,12px)]">
          <form
          ...
          >
            <input
              ...
              maxLength={500}
              className="flex-1 px-4 py-2.5 rounded-full bg-neutral-800 text-sm text-neutral-100 placeholder-neutral-400 outline-none border border-neutral-800 focus:border-neutral-400"
            />
            <button
              type="submit"
              disabled={!input.trim() || sending}
              className="p-2.5 rounded-full bg-white text-neutral-900 disabled:opacity-30 transition-opacity"
            >
