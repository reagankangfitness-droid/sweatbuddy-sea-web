...
function buildUserMessage(context: NudgeContext): string {
  switch (context.signalType) {
    ...
    case 'INACTIVITY_REENGAGEMENT':
      return `User "${context.userName || 'there'}" hasn't joined any fitness session in ${context.daysSinceLastActivity} days. Write a warm re-engagement nudge to bring them back.`
    ...
    default:
      return 'Write a friendly fitness session nudge.'
  }
}
...
const STATIC_FALLBACKS: Record<NudgeSignalType, NudgeCopy> = {
  EVENT_RECOMMENDATION: {
    ...
    body: 'Check out this new session from a community you\'ve joined before. Spots may be limited!',
  },
  INACTIVITY_REENGAGEMENT: {
    title: 'We miss you! Time to get moving?',
    body: 'It\'s been a while since your last activity. Browse upcoming sessions near you.',
  },
  LOW_FILL_RATE: {
